          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {analysisData && !isAnalyzing && (
              <>
                ...
                {(() => {
                  // Only show metrics that are actually calculated for single-page scans
                  const schemaScore = analysisData.ai?.schemaQuality?.score ?? 0
                  const schemaCount = analysisData.schemas?.length ?? 0
                  ...
                  // Metadata health: both title and description must be optimal length
                  const titleOptimal = titleLength >= 30 && titleLength <= 60
                  const descOptimal = metaLength >= 50 && metaLength <= 160
                  const metaHealth = (titleOptimal && descOptimal) ? 100 : 
                                     (titleLength > 0 && metaLength > 0) ? 50 : 0
                  
                  const statCards = [
                    { label: "Schema Types", value: schemaCount.toString(), color: schemaCount > 0 ? "text-geo" : "text-destructive", border: schemaCount > 0 ? "border-geo/30" : "border-destructive/30", bg: schemaCount > 0 ? "bg-geo/5" : "bg-destructive/5", tip: "Number of structured data types found on this page." },
                    ...
                    { label: "Metadata Health", value: `${metaHealth}%`, color: metaHealth === 100 ? "text-geo" : metaHealth === 50 ? "text-yellow-600" : "text-destructive", border: metaHealth === 100 ? "border-geo/30" : metaHealth === 50 ? "border-yellow-500/30" : "border-destructive/30", bg: metaHealth === 100 ? "bg-geo/5" : metaHealth === 50 ? "bg-yellow-500/5" : "bg-destructive/5", tip: "Title (30-60 chars) and description (50-160 chars) optimization." },
                    { label: "H1 Tag", value: h1Count === 1 ? "✓" : h1Count === 0 ? "✗" : `${h1Count}`, color: h1Count === 1 ? "text-geo" : "text-destructive", border: h1Count === 1 ? "border-geo/20" : "border-destructive/20", bg: h1Count === 1 ? "bg-geo/5" : "bg-destructive/5", tip: "H1 tag presence and uniqueness (should be exactly 1)." },
                    { label: "HTTPS", value: httpsStatus === 100 ? "✓" : "✗", color: httpsStatus === 100 ? "text-geo" : "text-destructive", border: httpsStatus === 100 ? "border-geo/20" : "border-destructive/20", bg: httpsStatus === 100 ? "bg-geo/5" : "bg-destructive/5", tip: "Secure HTTPS connection." },
                    { label: "Response Time", value: `${analysisData.structuralData?.responseTime ?? 0}ms`, color: (analysisData.structuralData?.responseTime ?? 0) < 1000 ? "text-geo" : "text-yellow-600", border: (analysisData.structuralData?.responseTime ?? 0) < 1000 ? "border-geo/30" : "border-yellow-500/30", bg: (analysisData.structuralData?.responseTime ?? 0) < 1000 ? "bg-geo/5" : "bg-yellow-500/5", tip: "Page load speed (under 1000ms is good)." },
                  ]

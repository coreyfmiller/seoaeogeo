...
export default function KeywordArenaV2Page() {
  ...
  useEffect(() => {
    ...
    const phases = [
      ...
      { at: 88, label: "Building leaderboard..." },
    ]
  ...
  const handleStartArena = () => {
    ...
    if (userSiteUrl.trim()) urls.unshift(userSiteUrl.trim())
    setPendingUrls(urls)
  ...
  const handleConfirmArena = async () => {
    ...
    try {
      const res = await fetch('/api/keyword-arena-v2', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleRetry = async (url: string) => {
    ...
    setRetryingUrl(url)
    try {
      const res = await fetch('/api/keyword-arena-v2/retry', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleReset = () => {
    ...
    setUserSiteUrl(""); setShowAddSite(false); setError(null)
  }
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-white/30">These sites will be crawled and AI-scored. Add your site to see where you rank.</CardDescription>
                    </div>
                ...
                <CardContent className="space-y-2">
                  {!showAddSite ? (
                    ...
                    <div className="flex gap-2 p-3 rounded-xl border border-[#00e5ff]/30 bg-[#00e5ff]/5">
                      ...
                      <button onClick={() => { setShowAddSite(false); setUserSiteUrl("") }} className="p-2 text-white/30 hover:text-white/60 transition-colors">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  )}
                  ...
                  {searchResults.map((result, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.12] transition-all">
                      <div className="h-8 w-8 rounded-lg bg-white/[0.06] flex items-center justify-center text-xs font-black text-white/40">#{result.rank}</div>
                      <div className="flex-1 min-w-0">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-white/30">
                        ...
                        {arenaResult.totalSites - arenaResult.scoredSites > 0 && (
                          <span className="text-yellow-500"> • {arenaResult.totalSites - arenaResult.scoredSites} failed</span>
                        )}
                      </p>
                    </div>
                  </div>
